...
def analizar(datos_raw):
    ...
    # Si el JSON está truncado, intentar repararlo cerrando estructura
    try:
        resultado = json.loads(texto)
    except json.JSONDecodeError:
        # Intentar cerrar JSON truncado añadiendo cierres
        for sufijo in [']}}}', ']}', '}}', '}']:
            try:
                resultado = json.loads(texto + sufijo)
                print("  Aviso: JSON reparado con sufijo de cierre")
                break
            except json.JSONDecodeError:
                continue
        else:
            # Si no se puede reparar, devolver estructura vacía
            print("  Aviso: JSON no parseable, devolviendo estructura vacía")
            resultado = {
                "fecha": fecha,
                "hay_alertas": False,
                "resumen_ejecutivo": "Error en análisis — revisar log",
                "alertas": {k: [] for k in ["estrenos","deportes","especiales","entrevistas","parrilla","records"]}
            }
